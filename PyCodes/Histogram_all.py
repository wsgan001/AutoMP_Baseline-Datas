...
def draw_Histogram():
    ...
    Y5 = [0.4326687414591507, 0.40982108491143937, 0.4022995440973937, 0.4104443894945374, 0.37144170020382455]  # ProxEmbed
    Y6 = [0.6688340708259806, 0.6274915826466256, 0.6720513709936207, 0.7457815125543222, 0.6974879943936839]  # D2AGE
    # fig, ax = plt.subplots()
    index = np.arange(5) + 1
    bar_width = 0.1
    opacity = 0.8
    rec1 = plt.bar(index, Y2, bar_width, alpha=opacity, color='blue', edgecolor='k', label='RelSim', hatch='/')
    rec2 = plt.bar(index + bar_width, Y5, bar_width, alpha=opacity, color='red', edgecolor='k',
                   label='ProxEmbed', hatch='\\')
    rec3 = plt.bar(index + 2*bar_width, Y6, bar_width, alpha=opacity, color='pink', edgecolor='k', label='D2AGE', hatch='|')
    rec4 = plt.bar(index + 3*bar_width, Y1, bar_width, alpha=opacity, color='darkgreen', edgecolor='k',
                   label='ESER', hatch='-')
    rec5 = plt.bar(index + 4*bar_width, Y4, bar_width, alpha=opacity, color='purple', edgecolor='k', label='RelSUE', hatch='.')
    rec6 = plt.bar(index + 5*bar_width, Y3, bar_width, alpha=opacity, color='white', edgecolor='k',
                   label='RelSUE-E', hatch='o')
    ...
    plt.legend(ncol=6, loc=(0.015, 1.05), fontsize=fontsz)
    ...
    Y5 = [0.4509009379243352, 0.41250269646362714, 0.4205603757061974, 0.4416644085892196, 0.40060976269770376]  # ProxEmbed
    Y6 = [0.648439483534206, 0.636849181879786, 0.675147537012236, 0.7357155117181435, 0.6978377284738132]  # D2AGE
    # fig, ax = plt.subplots()
    index = np.arange(5) + 1
    bar_width = 0.1
    opacity = 0.8
    ...
    plt.ylabel('NDCG@20', fontsize=fontsz)

    plt.subplot(2, 2, 2)

    Y1 = [0.4741883195687049, 0.555323940324946, 0.55043329042543, 0.5657737164464746, 0.572017569808552]  # ESER
    Y2 = [0.30788268998427304, 0.3380689216771812, 0.3564773197619133, 0.3501282016037503, 0.3382656183685123]  # RelSim
    ...
    Y5 = [0.5, 0.5, 0.5, 0.5, 0.5]  # ProxEmbed
    Y6 = [0.5, 0.5, 0.5, 0.5, 0.5]  # D2AGE
    # fig, ax = plt.subplots()
    index = np.arange(5) + 1
    bar_width = 0.1
    opacity = 0.8
    ...
    Y2 = [0.3126058087459059, 0.34665825450329674, 0.37032533363259573, 0.36379966375969586, 0.3562864265075326]  # RelSim
    ...
    plt.subplots_adjust(left=0.05,right=0.99,wspace=0.15,hspace=0.15,bottom=0.13,top=0.86)
    # plt.show()
